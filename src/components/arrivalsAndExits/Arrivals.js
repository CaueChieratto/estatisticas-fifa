import React from "react";
import "./styleTransfer.css";

export default function Arrivals() {
  return (
    <>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Daniaaaaaaaaaa Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victor</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Dani Olmo</div>
            <div className="infos">
              Transferencia <span className="number">€55M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Leipzig</div>
            <div className="infos">8 de ago. de 2024</div>
          </div>
        </div>
      </div>
      <div className="containerTransfers">
        <div className="containerInfos">
          <div className="infosPlayer">
            <div className="infosTitles">Pau Victorbbbbbbbbbb</div>
            <div className="infos">
              Transferencia <span className="number">€2.7M</span>
            </div>
          </div>
          <div className="infosTransfer">
            <div className="infosTitles">Girona</div>
            <div className="infos">23 de jul. de 2024</div>
          </div>
        </div>
      </div>
    </>
  );
}
